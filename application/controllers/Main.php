...
//                $order = 'price';
        ...
        $tpl_container['content']              = '';
        $tpl_container['breadcrumb']           = $this->load->view('cctv-tmp/component/breadcrumb_view', $data_ar, TRUE);
        $tpl_container['category_grid']        = $this->load->view('cctv-tmp/component/category_grid_2_view', $data_ar, TRUE);
        
        if( $page == 1 && $use_order == false && ($url_page_isset == false || $city != false) ) // условие для исключения дублей главных страниц категорий
        {
            $tpl_container['content']         .= $this->load->view('cctv-tmp/component/cat_txt_view', $data_ar, TRUE);
        }
        $tpl_container['goods_grid']    = $this->load->view('cctv-tmp/component/goods_grid_view', $data_ar, TRUE);
...
//        print_r($tpl_container);
        ...
        $tpl_ar['content'] = $this->load->view('cctv-tmp/page-container/category_view', $tpl_container, TRUE);
        
        $this->load->view('cctv-tmp/main_view', $tpl_ar );
    }
    
    function goods( $id = false, $url_name = false ){
//        if( $this->cache_do ) $this->output->cache(10);
        
        $id = (int) $id;
        if( !$id || !$url_name ){ show_404 (); exit(); }
        ...
        $data_ar['like_video']          = $this->goods->get_like_video($data_ar['goods_info']['id'],4);
...
//        $data_ar['left_articles_list']  = add_gname_to_larticles($data_ar['left_articles_list'], $data_ar['goods_info']['name']);
        ...
        $data_ar['rnd_descript']        = $this->goods->getRndDescription(  $data_ar['goods_info']['id'], 
                                                                            $data_ar['this_cat']['rnd_goods_descript_tpl_id'], 
                                                                            $data_ar['goods_info']['name'] );
        $data_ar['rnd_city_shipping']   = $this->goods->shippingRndTxt($data_ar['goods_info']['id'], $data_ar['goods_info']['name']);
...
//            $tpl_ar['left_banner']      = $this->load->view('component/banner/ip_online_v', '', TRUE);
        ...
        $data_ar['tpl_container']['like_goods']    = $this->load->view('cctv-tmp/component/like_goods_view', $data_ar, TRUE);
...
//        if( $this->cache_do ) $this->output->cache(60*24*10);
        ...
        if( $url_name == 'main' ) redirect('http://'.$_SERVER['HTTP_HOST'].'/', 'location', 301);
