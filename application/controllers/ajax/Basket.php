...
//            print_r($goods_ar);
        ...
        if( count($data_ar['goods_list']) >= 1 )
            $json_ar['content'] = $this->load->view('cctv-tmp/ajax/basket_view', $data_ar, TRUE);
        else
    ...
    function order(){
        ...
        if( isset($_COOKIE['basket']) ){ //подсчет суммы
            ...
            if( count($goods_ar) > 0 ){
                ...
                $json_ar['content'] = $this->load->view('cctv-tmp/ajax/order_view', array('summ'=>$summ), TRUE);
            }
        }
    ...
    function send_order(){
        if( isset($_COOKIE['basket']) ){
            ...
            if( count($goods_ar) > 0 ){
                ...
                $this->email->from('mail@cctv-pro.com.ua', 'CCTV Pro');
                $this->email->to($to_ar);
...
//                    echo $this->email->print_debugger();
    ...
    function update_menu(){
        ...
        $json_data['cnt_goods']     = '0' .' шт. -';
        $json_data['summ_price']    = '0.00' .' грн.';
        ...
        if( isset($_COOKIE['basket']) ){
            ...
            if( count($goods_ar) > 0 ){
                ...
                $json_data['cnt_goods']     = $count_goods .' шт. -';
                $json_data['summ_price']    = number_format($summ, 2, '.', ' ') .' грн.';
            }
        }
