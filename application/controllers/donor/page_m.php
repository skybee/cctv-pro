...
class page_m extends CI_Controller{
    ...
    function get_page_data( $url_name ){
        ...
        $query = $this->db->query("SELECT `title`, `text` FROM `articles` WHERE `id`!={$article_id} AND MATCH(`title`, `text`) AGAINST('".mysql_real_escape_string($rearch_str)."') LIMIT 10 ");
...
//                                  ");
        ...
        $goods_url = 'http://house-control.org.ua/goods/'.$row['id'].'/'.$row['url_name'].'/';
